...
class Mage_Checkout_MultishippingController extends Mage_Checkout_Controller_Action
{
    ...
    public function overviewPostAction()
    {
        ...
        try {
			...
			// support for payment methods behavior in Multishipping mode
			...
			foreach($ids as $order_id){
				...
				foreach($ids as $sibling_order_id){
					if($order_id != $sibling_order_id){
						$sibling_ids[] = $sibling_order_id;
					}
				}
				
				$order->addStatusHistoryComment('Siblings: '.implode('#',$sibling_ids));
				$order->save();
				break;
			}
			
			if('Eghl' == $payment_method_code){
				//////////// Multi-shipping support for eGHL  /////////////
				...
				foreach ($Eghl->getCheckoutFormFields($order->getIncrementId(),true) as $field=>$value) {
				...
				// Multishipping Default Behavior
